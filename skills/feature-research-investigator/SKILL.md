---
name: feature-research-investigator
description: Research how the current codebase works before a feature change. Use when a feature already has `./features/{feature-name}/PRD.md` and the user needs a `RESEARCH.md` documenting the current git hash, code evidence, clean/dirty file status, confidence scoring, and mapping between current behavior and PRD requirements.
---
